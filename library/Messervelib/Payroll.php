...
class Messervelib_Payroll {
	...
	public function save_the_day($employee_id, $group_id, $data) {
				...
				if($holiday_today && $holiday_today->getType() == 'legal') { // Unattended legal holiday
                    ...
                    if($legal_unattended_viable && $legal_unattended_group == $group_id) {
                        ...
                        if($group_id != $legal_unattended_group) {
                            $time_array += array('legal_unattend' => 0);
                        } else {
		...
		*/
                ...
                $PayrollToday->getMapper()->findOneByField(
                    array("attendance_id","date")
                    // , array($Attendance->getId(), substr($Attendance->getDatetimeStart())
                    , array($Attendance->getId(), $new_date_today)
                    , $PayrollToday
                );
	...
	protected function _process_bop ($attendance_id, Messerve_Model_Employee $Employee, $cutoff, $date) { // Bike ownership program
    ...
    public function GetEmployeePayroll($employee_id, $group_id, $period_start, $EmployeeModel ) { //, $date_start, $date_end
...
//             -- AND date BETWEEN '$date_start' AND '$date_end'
	...
	protected function _get_daily_fuel_purchase(Messerve_Model_Attendance $Attendance, $daily_total_duration) { // Retired
		$employee_id = $Attendance->getEmployeeId();
		$date_start = $Attendance->getDatetimeStart();
