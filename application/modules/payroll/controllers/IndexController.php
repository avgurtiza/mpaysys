...
class Payroll_IndexController extends Zend_Controller_Action
{
    ...
    public function payslipsAction()
    {
        ...
        $this->_compute(); $this->_compute(); // TODO:  Fix this!
        ...
        foreach ($this->_employee_payroll as $value) {
            ...
            */
            ...
            if(isset($value['more_income']) && $value['more_income']['misc_income'] > 0) {
                $dim_y -= 8;
        	...
        	if(isset($value['more_income']) && $value['more_income']['incentives'] > 0) {
	        	$dim_y -= 8;
            ...
            if(isset($value['more_income']) && $value['more_income']['thirteenth_month_pay'] > 0) {
	        	$dim_y -= 8;
            ...
            if($group_id == $Employee->getGroupId()) {
                if($BOPAttendance && $BOPAttendance->getMaintenanceAddition() > 0) {
                    $dim_y -= 8;
    ...
    */
	...
	public function exportAction() {  // CSV export
		$this->_helper->layout()->disableLayout();
		
		$period_covered = $this->_request->getParam('period_covered');
		
 		header('Content-type: text/csv');	header('Content-Disposition: attachment; filename="Payroll_report.csv"');
	...
	public function etpsAction() {  // CSV export
		$this->_helper->layout()->disableLayout();
