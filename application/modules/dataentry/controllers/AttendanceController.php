...
class Dataentry_AttendanceController extends Zend_Controller_Action
{
	...
	*/
						...
						if($employee_number > 0) {
							...
							if($data[11] != '' && $data[12] != '') {
								...
								if(
										...
										// || strpos($data[11], 'AM') > 0 && strpos($data[12], 'PM') > 0
								) {
									$date_now = date('Y-m-d', strtotime('+1 day', strtotime($data[0])));
								}
							...
							if($Employee) {
								...
								$this->_save_the_day($Attendance->getEmployeeId(), $Attendance->getGroupId(), array($data['date']=> $save_this_day));
                                 ...
                                 echo "<br />MISSING $employee_number:"; preprint($data);
								$missing[$data[6]] = $data;
                                echo "End MISSING";
							}
						}
	...
	public function searchrelieverAction() { // AJAX
	...
	public function addrelieverAction() { // AJAX
		$this->_helper->layout()->disableLayout();
		$this->_helper->viewRenderer->setNoRender(true);
    ...
    public function resetdayAction() { // AJAX
        $this->_helper->layout()->disableLayout();
        $this->_helper->viewRenderer->setNoRender(true);
