...
/**
 * Data Mapper implementation for Messerve_Model_BopAttendance
 *
 * @package Messerve_Model
 * @subpackage Mapper
 * @author Slide Gurtiza
 */
class Messerve_Model_Mapper_BopAttendance extends Messerve_Model_Mapper_MapperAbstract
{
    /**
     * Returns an array, keys are the field names.
     *
     * @param Messerve_Model_BopAttendance $model
     * @return array
     */
    public function toArray($model)
    {
        if (! $model instanceof Messerve_Model_BopAttendance) {
        ...
        $result = array(
            ...
            'maintenance_addition' => $model->getMaintenanceAddition(),
            'previous_month_hours' => $model->getPreviousMonthHours(),
        );
     ...
     *
     * @return Messerve_Model_DbTable_BopAttendance
     */
    public function getDbTable()
    {
        if ($this->_dbTable === null) {
            $this->setDbTable('Messerve_Model_DbTable_BopAttendance');
        }
     ...
     *
     * @param Messerve_Model_BopAttendance $model The model to delete
    ...
    public function delete($model)
    {
        ...
        try {
            ...
            $pk_val = $model->getBopId();
            if ($pk_val === null) {
                throw new Exception('The value for BopId cannot be null');
            } else {
                $where[] = $this->getDbTable()->getAdapter()->quoteInto('bop_id = ?', $pk_val);
            }
     ...
     */
    public function save(Messerve_Model_BopAttendance $model,
     ...
     * @param Messerve_Model_BopAttendance|null $model
     * @return Messerve_Model_BopAttendance|null The object provided or null if not found
     */
     ...
     * @param Messerve_Model_BopAttendance|null $entry The object to load the data into, or null to have one created
     * @return Messerve_Model_BopAttendance The model with the data provided
     */
    public function loadModel($data, $entry)
    {
        if ($entry === null) {
            $entry = new Messerve_Model_BopAttendance();
        }

        if (is_array($data)) {
            $entry->setBopId($data['bop_id'])
                  ...
                  ->setMaintenanceAddition($data['maintenance_addition'])
                  ->setPreviousMonthHours($data['previous_month_hours']);
