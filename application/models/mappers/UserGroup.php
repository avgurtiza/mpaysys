...
/**
 * Data Mapper implementation for Messerve_Model_UserGroup
 *
 * @package Messerve_Model
 * @subpackage Mapper
 * @author Slide Gurtiza
 */
class Messerve_Model_Mapper_UserGroup extends Messerve_Model_Mapper_MapperAbstract
{
    /**
     * Returns an array, keys are the field names.
     *
     * @param Messerve_Model_UserGroup $model
     * @return array
     */
    public function toArray($model)
    {
        if (! $model instanceof Messerve_Model_UserGroup) {
     ...
     *
     * @return Messerve_Model_DbTable_UserGroup
     */
    public function getDbTable()
    {
        if ($this->_dbTable === null) {
            $this->setDbTable('Messerve_Model_DbTable_UserGroup');
        }
     ...
     *
     * @param Messerve_Model_UserGroup $model The model to delete
    ...
    public function delete($model)
    {
        ...
        try {
            ...
            $pk_val = $model->getUserId();
            if ($pk_val === null) {
                throw new Exception('The value for UserId cannot be null');
            } else {
                $where[] = $this->getDbTable()->getAdapter()->quoteInto('user_id = ?', $pk_val);
            }
     ...
     */
    public function save(Messerve_Model_UserGroup $model,
     ...
     * @param Messerve_Model_UserGroup|null $model
     * @return Messerve_Model_UserGroup|null The object provided or null if not found
     */
     ...
     * @param Messerve_Model_UserGroup|null $entry The object to load the data into, or null to have one created
     * @return Messerve_Model_UserGroup The model with the data provided
     */
    public function loadModel($data, $entry)
    {
        if ($entry === null) {
            $entry = new Messerve_Model_UserGroup();
        }
